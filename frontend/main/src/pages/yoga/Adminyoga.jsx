...
import { getAllYogaSessionsApi, createYogaSessionApi, updateYogaSessionApi, deleteYogaSessionApi } from "../../api/api";
...
const AdminYoga = () => {
  ...
  const handleSave = async (id) => {
    try {
      ...
      const response = await updateYogaSessionApi(id, {
        title: session.title,
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await createYogaSessionApi({
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this session?')) {
      try {
        const response = await deleteYogaSessionApi(id);
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-100 via-yellow-100 to-pink-100 text-gray-800">
      {/* Sidebar */}
      <Adminsidebar />
      ...
      <div className="flex-1 p-8">
        ...
        {showCreateForm && (
          <div className="mb-8 bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-2xl font-semibold mb-4">➕ Create New Yoga Session</h2>
