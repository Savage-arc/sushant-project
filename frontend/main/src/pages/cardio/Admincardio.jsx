...
import { getAllCardioSessionsApi, updateCardioSessionApi, createCardioSessionApi, deleteCardioSessionApi } from "../../api/api";
...
const AdminCardio = () => {
  ...
  const handleSave = async (id) => {
    try {
      ...
      const response = await updateCardioSessionApi(id, {
        title: session.title,
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await createCardioSessionApi({
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this session?')) {
      try {
        const response = await deleteCardioSessionApi(id);
      ...
      <div className="flex-1 p-8">
        ...
        {showCreateForm && (
          <div className="mb-8 bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-2xl font-semibold mb-4">➕ Create New Cardio Session</h2>
